...
模型我都放在release里了，3个模型，表现最好的是模型2，模型3加入的psa注意力纯当练手，表现一般。三个模型我都是租云服务器上的显卡训练，4050核显带不动，第一次用yolo_train时，可以先把数据增强和注意力机制相关的代码注释掉，不然要跑很久，第三个模型哪怕租的是409048G也要从凌晨4点跑到当晚7点（数据集20000）网上好用的数据集比较难找，找到的数据集记得检查做数据清洗
